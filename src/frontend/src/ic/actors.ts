...
export const createEventBusActor = (opts: Opts = {}) =>
  Actor.createActor(eventBusIDL, {
    agent: makeAgent(opts.host),
    canisterId: EVENT_BUS_ID,
  });

export const createReputeActor = (opts: Opts = {}) =>
  Actor.createActor(reputeIDL, {
    agent: makeAgent(opts.host),
    canisterId: REPUTE_ID,
  });

export const createCollateralActor = (opts: Opts = {}) =>
  Actor.createActor(collateralIDL, {
    agent: makeAgent(opts.host),
    canisterId: COLLATERAL_ID,
  });

export const createTrustAIActor = (opts: Opts = {}) =>
  Actor.createActor(trustAiIDL, {
    agent: makeAgent(opts.host),
    canisterId: TRUST_AI_ID,
  });

export const createLoansActor = (opts: Opts = {}) =>
  Actor.createActor(loansIDL, {
    agent: makeAgent(opts.host),
    canisterId: LOANS_ID,
  });
