...
import { createActor as createEventBusActor, canisterId as EVENT_BUS_ID } from "../../declarations/event_bus_backend";
import { createActor as createReputeActor, canisterId as REPUTE_ID } from "../../declarations/repute_backend";
import { createActor as createCollateralActor, canisterId as COLLATERAL_ID } from "../../declarations/collateral_backend";
import { createActor as createTrustAIActor, canisterId as TRUST_AI_ID } from "../../declarations/trust_ai_backend";
import { createActor as createLoansActor, canisterId as LOANS_ID } from "../../declarations/loans_backend";
...
function Field(props: { label: string; value: string | number; onChange: (v: string) => void; type?: string; placeholder?: string }) {
  return (
...
function Area(props: { label: string; value: string; readOnly?: boolean }) {
  return (
    <label className="block">
      <span className="text-sm text-gray-700">{props.label}</span>
      <textarea
        className="mt-1 w-full rounded-xl border px-3 py-2 text-sm h-28"
        value={props.value}
...
function Btn(props: { onClick?: () => void; children: React.ReactNode; variant?: "primary" | "ghost"; disabled?: boolean }) {
  const base = "px-3 py-2 rounded-xl text-sm font-medium transition";
  const cls = props.variant === "ghost"
...
function pretty(v: unknown): string {
  try {
    ...
    return JSON.stringify(v, (_, val) => (typeof val === "bigint" ? `${val}n` : val), 2);
  } catch {
    return String(v);
  }
}
...
export default function DebugICPage() {
  ...
  const principal = useMemo(() => {
    try { return Principal.fromText(principalStr); } catch { return Principal.anonymous(); }
  }, [principalStr]);
  ...
  const eventBus = useMemo(() => createEventBusActor(EVENT_BUS_ID, { agentOptions: { host } }), [host]);
  const repute = useMemo(() => createReputeActor(REPUTE_ID, { agentOptions: { host } }), [host]);
  const collateral = useMemo(() => createCollateralActor(COLLATERAL_ID, { agentOptions: { host } }), [host]);
  const trustAI = useMemo(() => createTrustAIActor(TRUST_AI_ID, { agentOptions: { host } }), [host]);
  const loans = useMemo(() => createLoansActor(LOANS_ID, { agentOptions: { host } }), [host]);
      ...
      <div className="max-w-6xl mx-auto p-4 md:p-8 space-y-6">
        <header className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-gray-600">Quick calls into all five canisters</p>
          </div>
        ...
        <Card title="Shared Principal">
          <Field label="Principal (text)" value={principalStr} onChange={setPrincipalStr} placeholder="2vxsx-fae (anonymous)" />
          <div className="text-xs text-gray-600">Parsed: {principal.toText()}</div>
        </Card>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <Card title={`event_bus_backend  (id: ${EVENT_BUS_ID})`}>
            ...
            <SectionActions>
              <Btn onClick={async () => {
                ...
                } catch (e) { setEventsOut("ERROR: " + String(e)); }
              }}>emit + list_recent(10)</Btn>
              <Btn variant="ghost" onClick={async () => {
                try { const list = await eventBus.list_recent(10n); setEventsOut(pretty(list)); } catch (e) { setEventsOut("ERROR: " + String(e)); }
              }}>list_recent(10)</Btn>
            </SectionActions>
          ...
          <Card title={`repute_backend  (id: ${REPUTE_ID})`}>
            <SectionActions>
              <Btn onClick={async () => {
                try { const level = await repute.get_level(principal); setLevelOut(pretty(level)); }
                catch (e) { setLevelOut("ERROR: " + String(e)); }
              }}>get_level(principal)</Btn>
            </SectionActions>
          ...
          <Card title={`collateral_backend  (id: ${COLLATERAL_ID})`}>
            <Field label="Deposit amount (nat)" value={depositAmt} onChange={setDepositAmt} type="number" />
            <SectionActions>
              <Btn onClick={async () => {
                try {
                  await collateral.deposit_mock(principal, BigInt(depositAmt || "0"));
                  const bal = await collateral.get_collateral(principal);
                  setCollOut(pretty(bal));
                } catch (e) { setCollOut("ERROR: " + String(e)); }
              }}>deposit_mock + get_collateral</Btn>
              <Btn variant="ghost" onClick={async () => {
                try { const bal = await collateral.get_collateral(principal); setCollOut(pretty(bal)); }
                catch (e) { setCollOut("ERROR: " + String(e)); }
              }}>get_collateral</Btn>
            </SectionActions>
          ...
          <Card title={`trust_ai_backend  (id: ${TRUST_AI_ID})`}>
            <div className="grid grid-cols-2 gap-3">
              <Field label="Collateral (nat)" value={taCollateral} onChange={setTaCollateral} type="number" />
              <Field label="Trust (nat64)" value={taTrust} onChange={setTaTrust} type="number" />
            </div>
            <SectionActions>
              <Btn onClick={async () => {
                try {
                  ...
                  setTaOut(pretty(rec));
                } catch (e) { setTaOut("ERROR: " + String(e)); }
              }}>recommend(principal, collateral, trust)</Btn>
            </SectionActions>
          ...
          <Card title={`loans_backend  (id: ${LOANS_ID})`}>
            <div className="grid grid-cols-2 gap-3">
              <Field label="Loan amount (nat)" value={loanAmt} onChange={setLoanAmt} type="number" />
              <div />
            </div>
            <SectionActions>
              <Btn onClick={async () => {
                try { const r = await loans.ping(); setLoanOut(pretty(r)); }
                catch (e) { setLoanOut("ERROR: " + String(e)); }
              }}>ping()</Btn>
              <Btn onClick={async () => {
                try { const r = await loans.register_user(); setLoanOut(pretty(r)); }
                catch (e) { setLoanOut("ERROR: " + String(e)); }
              }}>register_user()</Btn>
              <Btn onClick={async () => {
                try { const s = await loans.get_summary(principal); setSummaryOut(pretty(s)); }
                catch (e) { setSummaryOut("ERROR: " + String(e)); }
              }}>get_summary(principal)</Btn>
              <Btn onClick={async () => {
                try { const d = await loans.request_loan(BigInt(loanAmt || "0")); setSummaryOut(pretty(d)); }
                catch (e) { setSummaryOut("ERROR: " + String(e)); }
              }}>request_loan(amount)</Btn>
            </SectionActions>
            <Area label="Last loan op result" value={loanOut} />
            <Area label="Summary / decision" value={summaryOut} />

            <div className="grid grid-cols-2 gap-3 pt-2">
              <Field label="Repay loan_id (nat)" value={repayLoanId} onChange={setRepayLoanId} type="number" />
              <Field label="Repay amount (nat)" value={repayAmt} onChange={setRepayAmt} type="number" />
            </div>
            <SectionActions>
              <Btn onClick={async () => {
                try {
                  const r = await loans.repay(BigInt(repayLoanId || "0"), BigInt(repayAmt || "0"));
                  setRepayOut(pretty(r));
                } catch (e) { setRepayOut("ERROR: " + String(e)); }
              }}>repay(loan_id, amount)</Btn>
            </SectionActions>
        ...
        <footer className="pt-4 text-xs text-gray-500">
          <div>Actors are created with host: <code>{host}</code></div>
          <div>BigInt is used for all nat/nat64 arguments to match Candid types.</div>
        </footer>
      </div>
    </div>
  );
